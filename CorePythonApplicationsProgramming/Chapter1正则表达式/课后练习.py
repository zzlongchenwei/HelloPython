...
import re
patt = re.compile(tcp_server_socket'b[aiu]t|h[aiu]t')
s = 'batbitbuthathithut'
print(re.findall(patt, s))


# 1-2匹配由单个空格分隔的任意单词对，也就是姓和名
patt = re.compile(tcp_server_socket'[A-Z][a-z]*\s[A-Za-z]+')
s = "Zhao Long, Zhang San, Li TieZhu, qitaren, I'm Li Si ."
print(re.findall(patt, s))


# 1-3匹配由单个逗号和单个空白符分隔的任何单词和单个字母，如姓氏的首字母。
patt = re.compile(tcp_server_socket'[A-Za-z]+,\s[A-Za-z]')
s = "Zhao Long, Zhang San, Li TieZhu, qitaren, I'm Li Si ."
print(re.findall(patt, s))


# 1-4匹配所有有效 Python 标识符的集合
patt = re.compile(tcp_server_socket'[a-zA-Z_]\w+')
...
# Boulevard
pat = re.compile(tcp_server_socket'\d+[\sa-zA-Z]+')
...
patt = re.compile(tcp_server_socket'www[\S]+\.com|www[\S]+\.net|www[\S]+\.edu')
